...
**Gotcha — multiple market cards can each have their own "SHOW ALL" button at once, confirmed live (2026-09-25).** With both Over/Under Total Goals and Correct Score expanded, 3+ "SHOW ALL" buttons can exist on the page simultaneously (one per card). An unscoped, page-wide `getByRole("button", { name: "SHOW ALL" }).first()` is ambiguous the moment more than one section has one rendered — confirmed as the real cause of a driver bug where Correct Score's SHOW ALL click actually expanded Over/Under's section instead, leaving the target scoreline still hidden. Confirmed scoping pattern, identical and symmetric for both Over/Under and Correct Score (same ancestor depth/classes for both): walking up from the market's own heading text match, the nearest ancestor containing a "SHOW ALL" descendant is at depth 3 (a plain wrapper `<div>`), with the visual "card" boundary at depth 5 (`tw-bg-sem-color-bg-container-lowest-default tw-w-full tw-rounded-n...`) and the shared page-wide `markets` container (where the SHOW ALL count jumps from 1 to several) at depth 6. Use the nearest-ancestor-containing-a-SHOW-ALL-descendant pattern (depth-agnostic, not a hardcoded class name — Tailwind utility classes could shift), same reasoning as the fixture-row-scoping fix in `driver.js`.
...
- It won't show up in a `fullPage: true` screenshot in a useful way (that mode doesn't composite `position: fixed` elements correctly across the stitched, scrolled captures — see gotcha below). Use a scoped element check (`browser evaluate` on `.bet-slip-container`, or `browser snapshot --selector '.bet-slip-container'`) to inspect its *content* reliably, not a screenshot.
- **Correction, confirmed live (2026-09-24), after this note's own original guidance caused a real bug:** the natural-seeming fix -- an *element-scoped* screenshot (`page.locator('.bet-slip-container').screenshot()`) -- is itself fundamentally broken for this widget in Playwright and reliably produces a completely blank capture, independent of leg count, timing, zoom, or actual on-page state (confirmed via three byte-identical blank captures across unrelated runs). A **plain default viewport screenshot** (`page.screenshot({ fullPage: false })`, i.e. neither element-scoped nor `fullPage: true`) captures this widget's real content correctly. Use that for driver screenshots going forward, not an element-scoped capture.
...
- **Screenshots come out with black bars on the right/bottom, confirmed and precisely quantified (2026-09-24).** These are raw CDP-attached tabs, not Playwright-launched pages, so `page.viewportSize()` is `null` and `page.screenshot()` falls back to a hardcoded/default `deviceScaleFactor` of 2 when sizing its capture buffer — but this profile's real effective DPR is ~1.333 (native 2.0 retina × the 67% Chrome zoom set by hand). Confirmed via CDP's `Page.getLayoutMetrics()`: real content paints into a ~1996×1824 device-pixel area, while an unclipped screenshot comes out 2994×2736 — exactly `cssLayoutViewport × 2`, not × the real 1.333, leaving the difference as unpainted black. Fix: clip the screenshot to the live CSS-pixel viewport size (`document.documentElement.clientWidth`/`clientHeight`, measured right before capture) instead of depending on or guessing the DPR — see `driver.js`'s `takeScreenshot()`.

## 9. Session/login recovery — confirmed live (2026-09-24)

The `betano` profile's login does **not** reliably survive overnight — observed logged-out (REGISTER/LOGIN visible instead of DEPOSIT) at the start of two separate sessions in this test cycle, with no code change or crash in between. Treat "confirm logged in" as a real per-session check, not a one-time setup step — see `driver.js`'s own README for where this fits in the run checklist.
...
1. Click the `LOGIN` button in the top nav: `page.getByRole("button", { name: "LOGIN" })`. This opens a **modal iframe**, not a same-page form — `#iframe-modal` wrapping `<iframe class="myaccount-iframe" src=".../en/myaccount/login">`. Scope everything below to that frame, e.g. `page.frameLocator("#iframe-modal iframe[src*='myaccount/login']")`.
2. Inside that frame, if Chrome has saved credentials for this profile, **both fields arrive pre-filled automatically** — nothing needs to be typed:
   - Username/email: `frame.getByRole("textbox", { name: "username" })`
   - Password: `frame.getByRole("textbox", { name: "Password" })`
   - Submit button: `frame.getByRole("button", { name: "submit" })` — note the accessible **name** is `"submit"`, not `"LOGIN"`, even though `"LOGIN"` is the visible label; match by role+name as given here, not by visible text.
3. Click submit. Reload the fixtures list afterward and confirm success **via a screenshot check, not an immediate post-reload state read** — `evaluate`-ing for a `DEPOSIT` button (or checking `window` state) right after `navigate` raced false at least twice during this session, catching the page mid-hydration before it had re-resolved auth. A screenshot taken a beat later reliably shows the true state (`DEPOSIT` + real balance = success; `REGISTER`/`LOGIN` still there = actually still logged out, not a race).

**Hard line — where this stops being something to automate:** the above is all "click a pre-filled autofill button," never typing or handling actual credentials. If the popup ever appears *without* both fields pre-filled — asking for a password, a 2FA code, a "confirm it's you" prompt, anything not already populated — that's a hard stop needing Winston directly, same category as a Cloudflare challenge. Don't type anything into that form under any circumstance; don't guess at completing it.

**Security note, not a Betano quirk:** the accessible-name/value pair Playwright (and any accessibility-tree snapshot) reads for the password field exposes its **real underlying value in plaintext**, regardless of the field's visual dot-masking. A snapshot taken to merely locate the submit button's ref will incidentally capture the actual password. Confirmed live — this is exactly how Winston's real password ended up in a session transcript on 2026-09-24. Avoid snapshotting that field at all once you already know the three selectors above; there's no need to re-discover them by inspecting the live form each time.

## 10. "Session Timer" popup — confirmed live and handled in code (2026-09-24)

Separate from the marketing bonus popup (section covered by `dismissMarketingPopup` in `driver.js`): a periodic responsible-gambling session-continuation prompt, likely SRIJ-mandated, with a live countdown to auto-logout if ignored. **A real occurrence during testing actually caused a real logout** — the countdown expired before it could be clicked by hand, proving the risk flagged here wasn't hypothetical.

Real selector, confirmed live from an actual specimen: `[data-test-id="session-timer-v3"]`, containing a plain `<button>CONTINUE</button>` (no `aria-label` needed) — `page.locator('[data-test-id="session-timer-v3"]').getByRole('button', { name: 'CONTINUE' })`.

Handled in `driver.js` via `dismissSessionTimer(page)`: called after every navigation alongside `dismissMarketingPopup` (idempotent no-op when not present, click immediately on detection since the countdown can be down to a handful of seconds by the time it's even noticed), **and** inside `pollForDecision`'s poll loop (every 20s) — since this popup can appear independently of navigation, with the tab otherwise completely idle while waiting on Winston's decision, which is exactly the scenario that caused the real logout above.
